    ...
    return (
      <>
        ...
        <aside className={`fixed left-0 top-16 h-[calc(100vh-4rem)] w-56 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-lg z-40 transform transition-transform duration-300 md:hidden ${
          ...
          <button
            onClick={closeSidebar}
            className="absolute top-4 right-4 p-2 rounded-md text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors"
          ...
          <nav className="flex flex-col gap-2 p-4 pt-12 flex-1">
            <NavLink
              ...
              className={({ isActive }) =>
                `${baseClasses} ${paddingClasses} ${isActive ? active : inactive}`
              }
            >
          ...
          {user && (
            <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700 px-4 pb-4">
              ...
              <button
                onClick={logout}
                className={`${baseClasses} ${paddingClasses} ${inactive} w-full text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300`}
              >
                <LogOut size={18} />
                <span>Sair</span>
              </button>
    ...
    <aside className={`${desktopOpen ? 'w-56' : 'w-16'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-sm h-full p-4 flex flex-col overflow-y-auto transition-all duration-300 hidden md:flex`}>
      ...
      <nav className="flex flex-col gap-2 flex-1">
        <NavLink
          to="/dashboard"
          className={({ isActive }) =>
            `${baseClasses} ${paddingClasses} ${isActive ? active : inactive} ${!desktopOpen ? 'justify-center' : ''}`
          }
          title={!desktopOpen ? "Dashboard" : ""}
        >
          <LayoutDashboard size={18} />
        ...
        <NavLink
          ...
          title={!desktopOpen ? "Chat" : ""}
        >
          <MessageSquare size={18} />
          {desktopOpen && <span>Chat</span>}
        </NavLink>
        ...
        <NavLink
          ...
          title={!desktopOpen ? "Eventos" : ""}
        >
          <Table size={18} />
          {desktopOpen && <span>Eventos</span>}
        </NavLink>
      </nav>
      ...
      {user && (
        <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            onClick={logout}
            className={`${baseClasses} ${paddingClasses} ${inactive} w-full ${!desktopOpen ? 'justify-center' : ''} text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300`}
            title={!desktopOpen ? "Sair" : ""}
          >
            <LogOut size={18} />
            {desktopOpen && <span>Sair</span>}
          </button>
        </div>
      )}
    </aside>
